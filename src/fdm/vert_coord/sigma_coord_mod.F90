module sigma_coord_mod
  ...
  public sigma_coord_calc_ph_eta
  public sigma_coord_calc_dphdt_eta
...
contains
  ...
  subroutine sigma_coord_init(num_eta, namelist_path)
    ...
    allocate(sigi(num_eta+1)); sigi = 0
    allocate(sig (num_eta  )); sig  = 0
    ...
    do k = 1, num_eta
      global_mesh%full_eta(k) = sig(k)
    end do
    do k = 1, num_eta + 1
      global_mesh%half_eta(k) = sigi(k)
    end do
  ...
  pure real(r8) function sigma_coord_calc_ph_eta(k, phs) result(res)
  ...
  end function sigma_coord_calc_ph_eta

  pure real(r8) function sigma_coord_calc_dphdt_eta(k, dphsdt) result(res)
  ...
  end function sigma_coord_calc_dphdt_eta
